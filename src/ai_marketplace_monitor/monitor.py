            ...
            for item_config in self.config.item.values():
                ...
                if (
                    ...
                    if start_at_list is not None and start_at_list:
                        ...
                        if self.logger:
                            self.logger.info(
                                f"""{hilight("[Schedule]", "info")} Scheduling to search for the crazy {item_config.name} every {humanize.naturaldelta(search_interval)} {'' if search_interval == max_search_interval else f'to {humanize.naturaldelta(max_search_interval)}'}"""
                            )
